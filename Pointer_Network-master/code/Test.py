...
from ActiveSearch import allocate_devices_to_area
from PtrNet import Node, Area
from PtrNet import VirtualRequest
import random
...
# 初始化带宽容量和厂区数量
areas = [Area(bandwidth_capacity=random.randint(10, 20)) for _ in range(10)]
# 带宽从10到20随机生成

# 定义虚拟网络请求
v_request = VirtualRequest(cpu_required=60, accuracy_required=0.7, bandwidth_required=20, lifetime=100, device_num=4)

nodes = [
    Node(index=0, area=areas[0], cpu_capacity=100, data_size=50, data_distribution=0.8),
    Node(index=1, area=areas[1], cpu_capacity=120, data_size=60, data_distribution=0.9),
    Node(index=2, area=areas[2], cpu_capacity=130, data_size=70, data_distribution=0.85),
    # 可以继续添加更多的节点... 取决你的需求
]
...
# 使用 allocate_devices_to_area 函数
selected_nodes = allocate_devices_to_area(areas[0], v_request, nodes)
...
selected_nodes = allocate_devices_to_area(areas[0], v_request, nodes)
if selected_nodes:
    print(f"成功分配了 {len(selected_nodes)} 个设备")
# 強制成功，先不考慮帶寬
# else:
#     print("分配失败，带宽不足或设备不足")

# 检查设备是否满足虚拟请求要求
def check_constraints(node, v_request):
    T = v_request.cpu_required / node.cpu_capacity  # 计算设备计算时间
    if T > v_request.lifetime:
        return False  # 超过生命周期
    if node.data_distribution < v_request.accuracy_required:
        return False  # 数据分布不满足准确率要求
    return True

# 当任务生命周期结束时，释放设备和带宽资源
def release_resources(selected_nodes, area):
    for node in selected_nodes:
        node.selected = False
    area.selected_devices -= len(selected_nodes)
